...
const ClientMenu = () => {
  ...
  if (!isNameSet) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--bg-app)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        <motion.div 
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          className="card"
          style={{ maxWidth: '400px', width: '100%', textAlign: 'center', padding: '3rem 2rem' }}
        >
          <div style={{ width: 64, height: 64, background: 'var(--primary-light)', borderRadius: 'var(--radius-full)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem', color: 'var(--primary)' }}>
            <Utensils size={32} />
          </div>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Bem-vindo!</h1>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-app)', paddingBottom: '100px' }}>
      {/* Header */}
      <header style={{ background: 'var(--surface)', padding: '1.25rem 1rem', borderBottom: '1px solid var(--border)', position: 'sticky', top: 0, zIndex: 10, boxShadow: 'var(--shadow-sm)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', maxWidth: '600px', margin: '0 auto' }}>
          <div>
            <h1 style={{ fontSize: '1.25rem', fontWeight: 800, color: 'var(--text-primary)' }}>Olá, {clientName}!</h1>
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Mesa {tableId}</p>
          </div>
          {currentOrderId && (
            <button 
              className="btn btn-secondary btn-sm"
              onClick={() => setView('status')}
              style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', borderRadius: 'var(--radius-full)', color: 'var(--primary)', borderColor: 'var(--primary-light)', background: 'var(--primary-light)' }}
            >
      ...
      <main style={{ maxWidth: '600px', margin: '0 auto', padding: '1.25rem 1rem' }}>
        ...
        {view === 'menu' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {/* Categories */}
            <div className="tabs" style={{ marginBottom: '1.25rem', width: '100%', overflowX: 'auto', flexWrap: 'nowrap' }}>
                    ...
                    return (
                      ...
                      >
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          {item.desc && (
                            <p style={{ fontSize: '0.72rem', color: 'var(--text-secondary)', marginTop: '0.2rem', lineHeight: 1.4, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                              {item.desc}
                            </p>
                          )}
                        </div>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                          <div>
                            ...
                            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', fontWeight: 600 }}>R$ {(item.price * item.quantity).toFixed(2)}</p>
                          </div>
                        </div>
                    ...
                    <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        <div>
                          ...
                          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{item.quantity}x R$ {item.price.toFixed(2)}</p>
                        </div>
                      </div>
